...
fn rev_add_child_or_children(one: bool) {
    ...
    assert_undo_redo(
        ...
        |world, _| {
            ...
            if one {
                parent_mut
                    .rev_add_one_related::<ChildOf>(new_child, MaybeLocation::caller())
                    .unwrap();
            } else {
                parent_mut
                    .rev_add_related::<ChildOf>([new_child], MaybeLocation::caller())
                    .unwrap();
            }
            new_child
        },
...
fn rev_detach_all_children() {
    ...
    assert_undo_redo(
        ...
        |world, _| {
            let mut parent_mut = world.entity_mut(parent);
            parent_mut
                .rev_detach_all_related::<ChildOf>(MaybeLocation::caller())
                .unwrap();
        },
        |world, _| {
            assert_eq!(world.get::<ChildOf>(child), Some(&ChildOf(parent)));
...
fn rev_detach_child() {
    ...
    assert_undo_redo(
        ...
        |world, _| {
            let mut parent_mut = world.entity_mut(parent);
            parent_mut
                .rev_remove_related::<ChildOf>([child2], MaybeLocation::caller())
                .unwrap();
        },
        |world, _| {
            assert_eq!(world.get::<ChildOf>(child1), Some(&ChildOf(parent)));
...
fn rev_despawn_children(forward_finalize: bool) {
    ...
    assert_undo_redo_finalize(
        ...
        |world, _| {
            world
                .entity_mut(parent)
                .rev_despawn_related::<Children>(MaybeLocation::caller())
                .unwrap();
        },
        backward_assert,
