...
pub fn plugin<const ROW: u64>(app: &mut App) {
    ...
    app.rev_add_systems(
        RevUpdate,
        // Use rev_in_set to add reversible systems to a set you intend to configure reversibly as
        // well.
        (system1, system2::<ROW>).rev_in_set(RevSet),
    )
    .rev_configure_sets(
        RevUpdate,
        RevSet
    ...
    .insert_resource(Signal::Default);
}
...
fn system1(meta: Res<RevMeta>, mut signal: ResMut<Signal>) {
    match meta.running_direction() {
        RevDirection::Forward { .. } => {
            ...
            *signal = Signal::DoSpawn;
        }
        RevDirection::BackwardLog => {
            assert_eq!(*signal, Signal::DidSpawn { at: meta.now() + 1 });
        }
        _ => {}
    }
}
...
fn system2<const ROW: u64>(meta: Res<RevMeta>, mut signal: ResMut<Signal>, mut commands: Commands) {
    match meta.running_direction() {
        RevDirection::Forward { meta_past_len } => {
            assert_eq!(*signal, Signal::DoSpawn);
