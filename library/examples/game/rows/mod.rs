...
mod rev_schedule; // reversible schedule configuration
mod rev_world; // reversible exclusive system, not using reversible commands

pub fn plugin(app: &mut App) {
    app.add_plugins((
        rev_command::plugin::<1>,
        rev_hook::plugin::<2>,
        rev_logs_drain::plugin::<3>,
        rev_logs_mut::plugin::<4>,
        rev_observer::plugin::<5>,
        rev_schedule::plugin::<6>,
        rev_world::plugin::<7>,
    ));
}
