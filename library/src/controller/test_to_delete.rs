...
struct Test<T> {
    ...
    /// applied queries before last
    queries: Vec<QueryB<T>>,
}

impl Test<usize> {
    fn forward_fast_to(
        &self,
        start_if_in_range: Option<(bool, &Self)>,
    ) -> (
        Option<ProgressQuery>,
        Result<&Self, RangeInclusive<Wrapping<Ticks>>>,
    ) {
        let range =
            Wrapping(*self.forward_fast_range.start())..=Wrapping(*self.forward_fast_range.end());
        match start_if_in_range {
            None => (
                Some(ProgressQuery::ForwardFastTo(*range.end() + Wrapping(1))),
                Err(range),
            ),
            Some((false, test)) => (Some(ProgressQuery::ForwardFastTo(*range.end())), Ok(test)),
            Some((true, test)) => (Some(ProgressQuery::ForwardFastTo(*range.start())), Ok(test)),
        }
    }
    fn log_to(
        &self,
        ...
        let range = Wrapping(*self.log_range.start())..=Wrapping(*self.log_range.end());
        match start_if_in_range {
            None => (
                Some(ProgressQuery::LogTo(*range.end() + Wrapping(1))),
                Err(range),
            ),
            Some((false, test)) => (Some(ProgressQuery::LogTo(*range.end())), Ok(test)),
            Some((true, test)) => (Some(ProgressQuery::LogTo(*range.start())), Ok(test)),
        }
    }
    fn log_fast_to(
        &self,
        ...
        match start_if_in_range {
            None => (
                Some(ProgressQuery::LogFastTo(*range.end() + Wrapping(1))),
                Err(range),
            ),
            Some((false, test)) => (Some(ProgressQuery::LogFastTo(*range.end())), Ok(test)),
            Some((true, test)) => (Some(ProgressQuery::LogFastTo(*range.start())), Ok(test)),
        }
    }
    fn get_query(
        &self,
        ...
        match &self.queries[index] {
            QueryB::None(test) => (None, Ok(test)),
            QueryB::Forward(test) => (Some(ProgressQuery::Forward), Ok(test)),
            QueryB::ForwardFastToRangeStart(test) => self.forward_fast_to(Some((true, test))),
            QueryB::ForwardFastToRangeEnd(test) => self.forward_fast_to(Some((false, test))),
            QueryB::ForwardFastToOutOfRange => self.forward_fast_to(None),
            QueryB::LogToRangeStart(test) => self.log_to(Some((true, test))),
            QueryB::LogToRangeEnd(test) => self.log_to(Some((false, test))),
            QueryB::LogToOutOfRange => self.log_to(None),
            QueryB::LogFastToRangeStart(test) => self.log_fast_to(Some((true, test))),
            QueryB::LogFastToRangeEnd(test) => self.log_fast_to(Some((false, test))),
            QueryB::LogFastToOutOfRange => self.log_fast_to(None),
            QueryB::Pause(test) => (Some(ProgressQuery::Pause), Ok(test)),
        }
    }
}
...
impl SubVariation {
    ...
    fn assert_ranges_try_query(
        ...
        if let Some(progress_query) = self.progress_query {
            *expected = controller.query_progress_command(&mut commands, progress_query);
            if expected.is_err() {
                assert_eq!(
                    test, tests,
...
enum QueryB<T> {
    None(T),
    Forward(T),
    ForwardFastToRangeStart(T),
    ForwardFastToRangeEnd(T),
    ForwardFastToOutOfRange,
    LogToRangeStart(T),
    LogToRangeEnd(T),
    LogToOutOfRange,
    LogFastToRangeStart(T),
    LogFastToRangeEnd(T),
    LogFastToOutOfRange,
    Pause(T),
}
...
struct TestResource{
    ...
    current_test: usize,
    next_test: Option<usize>,
    path_index_if_at_first_ran: Option<usize>,
    path: Vec<(usize, usize)>, //index, len
    restart: bool,
    finished: bool,
}

impl TestResource{
    fn try_query(&mut self, at_first_ran: bool, controller: &Controller, commands: &mut Commands<'_, '_>){
        if at_first_ran != self.path_index_if_at_first_ran.is_some(){
            return;
        }
        let index = self.path.last().unwrap().0;
        let (query, expected) = self.current_test.get_query(index);
        if let Some(query) = query{
            match (controller.query_progress_command(commands, query), expected){
                (Ok(()), Ok(next)) => self.next_test = Some(next),
                (Err(r1), Err(r2)) => assert_eq!(r1, r2, "debug:\n{:#?}", controller.debug),
                (Err(r), Ok(..)) => panic!("Test should not result in Err({r:?}), debug:\n{:#?}", controller.debug),
                (Ok(()), Err(r)) => panic!("Test should result in Err({r:?}), debug:\n{:#?}", controller.debug),
            }
        }
        else if let Ok(next) = expected{
            self.next_test = Some(next);
        }
        else {
            unreachable!("Test::get_query should not return (None, Err)");
        }
    }
    fn setup_next(&mut self){

    }
    fn before_first_ran(mut test: ResMut<'_, Self>, controller: Res<'_, Controller>, mut commands: Commands<'_, '_>){
        assert_eq!(test.current_test.progress_current, controller.progress_current, "debug:\n{:#?}", controller.debug);
        test.try_query(false, &controller, &mut commands);
    }
    fn at_first_ran(mut test: ResMut<'_, Self>, controller: Res<'_, Controller>, mut commands: Commands<'_, '_>){
        test.try_query(true, &controller, &mut commands);
        ...
        match (check_time_stamp, test.current_test.time_stamp){
            (true, Some(time_stamp)) => assert_eq!(controller.time_stamp, Wrapping(time_stamp), "debug:\n{:#?}", controller.debug),
            (true, None) => panic!("time_stamp should be Some at this test, debug:\n{:#?}", controller.debug),
            (false, Some(..)) => panic!("time_stamp should be None at this test, debug:\n{:#?}", controller.debug),
            (false, None) => {}
        }
    }
    fn after_first_ran(mut test: ResMut<'_, Self>, controller: Res<'_, Controller>, mut log: ResMut<'_, TestLogCollection>){
        let log = take(&mut log.log);
        let commands_check: Vec<Command> = test.current_test.commands
            .iter()
...
impl TestRunner{
    const CONSTANTS: ControllerConsts = ControllerConsts::new(
    ...
    fn update(&mut self, test: Test){
        self.app.insert_resource((test, self.at_first_ran));
        self.app.update();
    }
    fn init_app() -> App{
        let mut app = App::new();
        ...
        Controller::into_world(
            Wrapping(0),
            VecDeque::with_capacity(Self::CONSTANTS.log_capacity),
            Self::CONSTANTS,
            &mut app.world,
        );
        app
...
impl RunTests for Vec<(Test, Option<usize>)> {
    fn run(self, expected: Result<(), RangeInclusive<Wrapping<Ticks>>>) {
    ...
    fn sub_run(
        ...
        let (mut before_first, (mut after_first, mut after_last)): (Vec<_>, (Vec<_>, Vec<_>)) =
            self.into_iter()
                .map(|step| {
                    let progress_query = step.1.map(|index| step.0.get_query(index));
                    let sub_variation = SubVariation::new(
                        step.0.forward_fast_range,
                        step.0.log_range,
                        progress_query,
                    );
...
enum BranchEnd {
    ExpectedError(RangeInclusive<Wrapping<Ticks>>),
    Queries(HashMap<ProgressQuery, Self>),
}
...
enum TestNode {
    Branch {
        ...
        test: Test,
        queries: [Option<(usize, ProgressQuery)>; Query::QueryPause as usize + 1],
    },
    Leaf {
        previous_node: usize,
        previous_query: usize,
