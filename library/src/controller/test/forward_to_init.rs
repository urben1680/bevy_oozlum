...
#[test]
fn forward_to_init_query_none_to_end() {
    [
        Test {
            time_stamp: Some(1),
            progress_current: Progress::Forward,
...
fn forward_to_init_query_forward_to_present() {
    forward_to_init_query(ProgressQuery::ForwardFastTo(Wrapping(2)));
}

#[test]
fn forward_to_init_query_forward_to_one_tick() {
    forward_to_init_query(ProgressQuery::ForwardFastTo(Wrapping(3)));
}

#[test]
fn forward_to_init_query_forward_to_two_ticks() {
    forward_to_init_query(ProgressQuery::ForwardFastTo(Wrapping(4)));
}

#[test]
fn forward_to_init_query_forward_to_too_many_ticks() {
    forward_to_init_query(ProgressQuery::ForwardFastTo(Wrapping(5)));
}
...
fn forward_to_init_query_log_to_past() {
    forward_to_init_query(ProgressQuery::LogFastTo(Wrapping(1)));
}

#[test]
fn forward_to_init_query_log_to_present() {
    forward_to_init_query(ProgressQuery::LogFastTo(Wrapping(2)));
}

#[test]
fn forward_to_init_query_log_to_future() {
    forward_to_init_query(ProgressQuery::LogFastTo(Wrapping(3)));
}
