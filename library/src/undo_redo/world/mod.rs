...
pub trait RevWorld {
    ...
    #[track_caller]
    fn rev_log_scope(&mut self, meta_past_len: MetaPastLen, entity: Entity);
    ...
    fn rev_despawn_single(&mut self, meta_past_len: MetaPastLen, entity: Entity) -> bool;
    ...
    fn rev_spawn_batch<I>(&mut self, meta_past_len: MetaPastLen, iter: I) -> Arc<[Entity]>
    where
        I: IntoIterator,
        I::Item: Bundle<Effect: NoBundleEffect>;

    /// Reversible version of [`World::spawn_empty`].
    fn rev_spawn_empty(&mut self, meta_past_len: MetaPastLen) -> EntityWorldMut;
...
impl RevWorld for World {
    fn redo_and_buffer(&mut self, meta_past_len: MetaPastLen, mut undo_redo: impl UndoRedo) {
        // todo: pass location
        undo_redo.redo(self);
        self.buffer_undo_redo(meta_past_len, undo_redo)
    }
    ...
    #[track_caller]
    fn rev_despawn_single(&mut self, meta_past_len: MetaPastLen, entity: Entity) -> bool {
        self.rev_try_despawn_single(meta_past_len, entity)
    ...
    fn rev_get_resource_or_init<R: Resource + FromWorld>(
        ...
        self.rev_init_resource::<R>(meta_past_len);
        self.resource_mut::<R>()
    }
    ...
    fn rev_get_resource_or_insert_with<R: Resource>(
        ...
        if !self.contains_resource::<R>() {
            self.buffer_undo_redo(meta_past_len, ResourceSwap::<R>(None));
        }
    ...
    fn rev_init_resource<R: Resource + FromWorld>(
        ...
        rev_init_resource_with_caller::<R>(self, meta_past_len, MaybeLocation::caller())
    }

    #[track_caller]
    fn rev_insert_resource<R: Resource>(&mut self, meta_past_len: MetaPastLen, resource: R) {
        rev_insert_resource_with_caller(self, meta_past_len, resource, MaybeLocation::caller())
    }
    ...
    fn rev_remove_resource<R: Resource, Out>(
        ...
        rev_remove_resource_with_caller(self, meta_past_len, c, MaybeLocation::caller())
    }
    ...
    fn rev_spawn<T: Bundle<Effect: NoBundleEffect>>(
        ...
        rev_spawn_with_caller_world(self, meta_past_len, bundle, MaybeLocation::caller())
    }
    ...
    where
        ...
        I::Item: Bundle<Effect: NoBundleEffect>,
    {
        rev_spawn_batch_with_caller(self, meta_past_len, iter, MaybeLocation::caller())
    }

    #[track_caller]
    fn rev_spawn_empty(&mut self, meta_past_len: MetaPastLen) -> EntityWorldMut {
        rev_spawn_empty_with_caller(self, meta_past_len, MaybeLocation::caller())
    }
    ...
    fn rev_try_despawn_single(
        ...
        rev_try_despawn_single_with_caller_world(
            self,
            meta_past_len,
            entity,
            MaybeLocation::caller(),
...
pub(crate) fn rev_init_resource_with_caller<R: Resource + FromWorld>(
    ...
    if !world.contains_resource::<R>() {
        world.buffer_undo_redo(meta_past_len, ResourceSwap::<R>(None));
    }
...
pub(crate) fn rev_insert_resource_with_caller<R: Resource>(
    ...
    world.buffer_undo_redo(meta_past_len, swap);
}
...
pub(crate) fn rev_remove_resource_with_caller<R: Resource, Out>(
    ...
    world.remove_resource::<R>().map(|resource| {
        let out = c(&resource);
        world.buffer_undo_redo(meta_past_len, ResourceSwap(Some(resource)));
        out
    })
}
...
pub(crate) fn rev_spawn_with_caller_world<B: Bundle<Effect: NoBundleEffect>>(
    ...
    rev_spawn_despawn_with_caller::<true>(&mut entity_mut, meta_past_len, caller);
    entity_mut
}
    ...
    I::Item: Bundle<Effect: NoBundleEffect>,
{
    ...
    cleaner.log_spawn_batch(&entities, caller, meta_past_len);
    cleaner.log_spawn_buffer_batch_reserve(entities.len(), caller, meta_past_len);
...
pub(crate) fn rev_spawn_empty_with_caller(
    ...
    rev_spawn_empty_inner(&mut entity_mut, meta_past_len, caller);
    entity_mut
}
...
pub(crate) fn rev_try_despawn_single_with_caller_world(
    ...
    match world.get_entity_mut(entity) {
        Ok(entity_mut) => super::entity_world::rev_try_despawn_single_with_caller(
            entity_mut,
            meta_past_len,
            caller,
        )?,
